...
interface NoteDao {
    ...
    @Query("SELECT * FROM notes WHERE LOWER(title) LIKE LOWER(:query) OR LOWER(content) LIKE LOWER(:query) ORDER BY created_at DESC")
    fun searchNotes(query: String): Flow<List<Note>>

    // Get notes containing a specific tag (case-insensitive, checks within comma-separated list)
    // Note: This uses LIKE which might not be the most efficient for large datasets or complex tag structures.
    // Consider normalizing tags into a separate table for better performance if needed.
    @Query("SELECT * FROM notes WHERE LOWER(tags) LIKE LOWER('%' || :tag || '%') ORDER BY created_at DESC")
